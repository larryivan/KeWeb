    ...
    <div class="relative mx-auto max-w-6xl px-4 py-24 sm:px-6">
      <div class="flex flex-col items-center justify-center text-center">
        ...
        <h1 class="text-4xl font-bold tracking-tight text-amber-900 dark:text-amber-200 md:text-5xl lg:text-6xl">
          {{ hero.title || "Ancient DNA Research" }}
        </h1>
    ...
    <div class="space-y-12">
      ...
      <section v-if="featuredPosts && featuredPosts.length > 0">
        <div class="flex items-center mb-6 border-b border-amber-100 dark:border-amber-900/30 pb-2">
          <h2 class="text-lg font-medium text-amber-700 dark:text-amber-400">Featured Research</h2>
        </div>
      ...
      <section>
        <div class="flex items-center mb-6 border-b border-amber-100 dark:border-amber-900/30 pb-2">
          <h2 class="text-lg font-medium text-amber-700 dark:text-amber-400">Recent Discoveries</h2>
        </div>
...
import { data as allPosts } from "../utils/article.data.js";
...
const featuredPosts = computed(() => {
  ...
  return features.value
    .map((url: string) => allPosts.find((post) => post.url === url))
    .filter(Boolean);
});
...
const filteredPosts = computed(() => {
  ...
  return allPosts.filter(post => !featuredUrls.includes(post.url));
});
</script>
